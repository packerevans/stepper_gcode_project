...
# === CONFIGURATION ===
# Define the path where designs are stored relative to the app root
...
def save_design():
    """Saves G-code to the designs folder on the Pi"""
    data = request.json
    filename = data.get("filename")
    gcode = data.get("gcode")

    if not filename or not gcode:
        return jsonify(success=False, error="Missing filename or G-code"), 400
    ...
    filepath = os.path.join(DESIGNS_FOLDER, clean_name)
    
    try:
        with open(filepath, "w") as f:
            f.write(gcode)
        
        log_message(f"Design Saved: {clean_name}")
        return jsonify(success=True, message=f"Saved as {clean_name}")
    except Exception as e:
