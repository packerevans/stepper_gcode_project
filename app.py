...
def update_firmware():
    ...
    try:
        ...
        subprocess.run(["arduino-cli", "upload", "-p", "/dev/ttyUSB0", "--fqbn", "arduino:avr:uno", ARDUINO_PROJECT_PATH], check=True)
        connect_arduino()
        return jsonify(success=True, message="Firmware Updated")
    except Exception as e: 
        connect_arduino()
